...
@Component({
  selector: 're-store-header-search',
...
export class HeaderSearchComponent implements OnInit, OnDestroy {
  ...
  ngOnDestroy(): void {
    document
      .querySelector('body')
      .removeEventListener('click', this.closeSearchForm);
  }
  ...
  searchProducts(event: any) {
    ...
    if (this.searchTerm.length > 2) {
      ...
      this.timer = setTimeout(() => {
        this.apiService
          .fetchHeaderSearchData(this.searchTerm)
          .subscribe((result) => {
            this.suggestions = result.products.reduce((acc, cur) => {
              let max = 0;
              let min = 99999;
  ...
  goProductPage() {
    this.searchTerm = '';
    this.suggestions = [];
    var inputElement: any = document.querySelector(
  ...
  submitSearchForm(e: Event) {
    e.preventDefault();
    this.router.navigate(['/shop/sidebar/3cols'], {
